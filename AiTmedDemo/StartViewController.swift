    ...
    @objc func continueButtonTapped(button: TYButton) {
        if mode == .initial {
            ...
            if AiTmed.hasCredential(for: phoneNumber) {//has credential
                ...
                AiTmed.sendOPTCode(args: AiTmedSDK.SendOPTCodeArgs(phoneNumber: phoneNumber)) { (result) in
            ...
            AiTmed.login(args: AiTmedSDK.LoginArgs(phoneNumber: phoneNumber, password: password)) { (result) in
                switch result {
                case .failure(let error):
                    DispatchQueue.main.async {
                        self.ty.displayAlert(title: "Error", msg: error.localizedDescription, hasCancel: false, action: {})
                ...
                case .success(_):
                    DispatchQueue.main.async {
                        self.dismiss(animated: true, completion: {
                            getRootViewController()?.ty.displayAlert(title: "Success", msg: "Welcome to prynote!", hasCancel: false, action: {})
                        })
                    }
                }
            }
            ...
            AiTmed.createUser(args: AiTmedSDK.CreateUserArgs(phoneNumber: phoneNumber, password: password, code: code)) { (result) in
                switch result {
                case .failure(let error):
                    DispatchQueue.main.async {
                        self.ty.displayAlert(title: "Error", msg: error.localizedDescription, hasCancel: false, action: {})
